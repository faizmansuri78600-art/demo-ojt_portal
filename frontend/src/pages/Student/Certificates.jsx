...
// Cosmetic accents cycled per-card since the backend doesn't store styling.
const THUMB_STYLES = [
  { border: "border-blue-300", accent: "text-blue-600", seal: "bg-amber-400" },
  { border: "border-emerald-400", accent: "text-emerald-600", seal: "bg-amber-400" },
  { border: "border-fuchsia-300", accent: "text-fuchsia-600", seal: "bg-rose-400" },
];
...
export default function Certificates() {
  ...
  useEffect(() => {
    ...
    async function fetchCertificates() {
      ...
      try {
        ...
        if (!cancelled) {
          const withStyle = (res.certificates || []).map((c, idx) => ({
            ...c,
            ...THUMB_STYLES[idx % THUMB_STYLES.length],
          }));
          setCertificates(withStyle);
        }
      } catch (err) {
        if (!cancelled) {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [certificates, search, statusFilter, programFilter, sortOrder]);

  const handleDownload = async (cert) => {
    setDownloadingId(cert._id);
    try {
      await api.downloadFile(`/certificates/${cert._id}/download`, `${cert.id}.pdf`);
      setCertificates((prev) =>
        prev.map((c) => (c._id === cert._id ? { ...c, downloaded: true } : c))
      );
    } catch (err) {
      setError(err.message || "Couldn't download that certificate.");
    } finally {
    ...
    <main className="px-8 pb-12 pt-7">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={stats.latest ? stats.latest.completionDate : "—"}
          ...
          >
            ...
            <div className="flex flex-row flex-wrap gap-2 lg:flex-col lg:items-start">
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg border border-blue-100 bg-white px-3 py-1.5 text-[13px] font-semibold text-blue-600 hover:bg-blue-50 disabled:opacity-50"
              >
                <Download size={15} />
